...
def eval_all(expressions, env):
    ...
    #     return complete_eval(scheme_eval(expressions.first, env, True))
    # else:
    #     complete_eval(scheme_eval(expressions.first, env, True))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Incorrect number of argument values")
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        env_func =lambda x: scheme_eval(x, env)
        b = operands.map(env_func)
        return scheme_apply(self, b, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Incorrect number of parameters passed")
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        # env_func =lambda x: Thunk(x, env)
        # dethunker = lambda x: complete_eval(x)
        # # # new_operands = Thunk(operands, env)
        # b = operands.map(env_func)
        print(scheme_apply(self, operands, env))
        return scheme_eval(complete_eval(scheme_apply(self, operands, env)), env)
        # scheme_eval(scheme_apply(self, b.map(dethunker), env), env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        env.define(target, complete_eval(scheme_eval(expressions.second.first, env, True)))
        return target
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    # if expressions is nil:
    #     return True
    # expr = complete_eval(scheme_eval(expressions.first, env, True))
    # if scheme_falsep(expr):
    #     return False
    # elif scheme_truep(expr) and expressions.second is nil:
    #     return expr
    # else:
    #     return do_and_form(expressions.second, env)
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            test = complete_eval(scheme_eval(clause.first, env, True))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        curr = bindings.first
        check_form(curr, 2, 2)
        symbols_list = Pair(curr.first, symbols_list)
        expr_list = Pair(complete_eval(scheme_eval(curr.second.first, env, True)), expr_list)
        bindings = bindings.second
    check_formals(symbols_list)
    return env.make_child_frame(symbols_list, expr_list)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            procedure = complete_eval(scheme_eval(first, env, True))
            check_procedure(procedure)
